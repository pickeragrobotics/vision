...
class LiveDetectorConfig:
    ...
    def from_node(cls, node: Node) -> "LiveDetectorConfig":
        node.declare_parameter(
            "color_topic",
            "/camera/color/image_raw",
            ParameterDescriptor(description="Color image topic to subscribe to."),
        )
        node.declare_parameter(
            "depth_topic",
            "/camera/depth/image_raw",
            ParameterDescriptor(description="Aligned depth image topic to subscribe to."),
        )
        node.declare_parameter(
            "camera_info_topic",
            "/camera/color/camera_info",
            ParameterDescriptor(
                description="CameraInfo topic used to extract color camera intrinsics."
            ),
        )
        node.declare_parameter(
            "flip_image",
        ...
        node.declare_parameter(
            "processing_period_sec",
            0.001,
            ParameterDescriptor(description="Timer period used to process the latest synchronized frame pair."),
        )
        node.declare_parameter(
            "log_every_n_frames",
            30,
    ...
    def validate(self) -> None:
        if not self.color_topic:
            raise ValueError("Parameter 'color_topic' must not be empty.")
        if not self.depth_topic:
            raise ValueError("Parameter 'depth_topic' must not be empty.")
        if not self.camera_info_topic:
            raise ValueError("Parameter 'camera_info_topic' must not be empty.")
        if not self.detection_topic:
            raise ValueError("Parameter 'detection_topic' must not be empty.")
        if not self.detection_3d_topic:
            raise ValueError("Parameter 'detection_3d_topic' must not be empty.")
